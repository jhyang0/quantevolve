...
def add_moving_averages(df, periods=None, column='close', ma_type=MAType.SMA):
    ...
    if price_data is None:
        raise ValueError(f"Column '{column}' not found in DataFrame and no 'close' column available")
...
def add_ichimoku(df, tenkan_period=9, kijun_period=26, senkou_span_b_period=52):
    ...
    result_df['ichimoku_senkou_a'] = (result_df['ichimoku_tenkan'] + result_df['ichimoku_kijun']) / 2
...
def add_bollinger_bands(df, period=20, std_dev=2, column='close'):
    ...
    result_df[f'bb_middle_{period}'] = result_df[column].rolling(window=period, min_periods=1).mean()
...
def add_stochastic(df, k_period=14, d_period=3, smooth_k=3):
    ...
    if smooth_k > 1:
        result_df['stoch_%k'] = result_df['stoch_%k_raw'].rolling(window=smooth_k, min_periods=1).mean()
    else:
...
def add_ichimoku(df, tenkan_period=9, kijun_period=26, senkou_b_period=52, chikou_period=26):
    ...
    result_df['ichimoku_senkou_a'] = ((result_df['ichimoku_tenkan'] + result_df['ichimoku_kijun']) / 2).shift(kijun_period)
...
def add_volume_indicators(df, period=20):
    ...
    result_df[f'volume_sma_{period}'] = result_df['volume'].rolling(window=period, min_periods=1).mean()
    
    # Volume Ratio (current volume / average volume)
    result_df['volume_ratio'] = result_df['volume'] / result_df[f'volume_sma_{period}'].replace(0, np.finfo(float).eps)
