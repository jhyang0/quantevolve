...
from ..utils.indicators import get_available_indicators_for_data, create_indicator_summary
...
class EnhancedQuantEvaluator:
    ...
    def analyze_strategy_code(self, strategy_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["has_multi_timeframe"] = any(
                term in code_content.lower()
                for term in ["timeframe", "period.*period", "fast.*slow"]
            )

            analysis["has_risk_management"] = any(
                term in code_content.lower()
                for term in ["atr", "stop", "risk", "drawdown", "volatility"]
            )

            analysis["has_position_sizing"] = any(
                term in code_content.lower()
                for term in ["position_size", "sizing", "capital", "allocation"]
            )
    ...
    def _calculate_complexity_score(self, code_content: str, analysis: Dict[str, Any]) -> float:
        ...
        if "params" in code_content and (
            "len(params)" in code_content or "if params" in code_content
        ):
            score += 5
    ...
    def _calculate_basic_metrics(self, signals: pd.Series) -> Dict[str, Any]:
        ...
        if not isinstance(strategy_returns, pd.Series):
            strategy_returns = pd.Series(strategy_returns, index=self.market_data_df.index).fillna(
                0.0
            )
    ...
    def _calculate_enhanced_metrics(
        self, signals: pd.Series, strategy_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_indicator_score(self, strategy_analysis: Dict[str, Any]) -> float:
        ...
        used_valuable = [
            ind
            for ind in strategy_analysis.get("indicators_used", [])
            if ind in valuable_indicators
        ]
    ...
    def _calculate_innovation_score(self, strategy_analysis: Dict[str, Any]) -> float:
        ...
        used_advanced = [
            ind
            for ind in strategy_analysis.get("indicators_used", [])
            if ind in advanced_indicators
        ]
        score += len(used_advanced) * 10

        # Bonus for pattern recognition
        pattern_indicators = [
            ind for ind in strategy_analysis.get("indicators_used", []) if ind.startswith("CDL")
        ]
    ...
    def _calculate_risk_adjusted_score(
        ...
        used_volatility = [
            ind
            for ind in strategy_analysis.get("indicators_used", [])
            if ind in volatility_indicators
        ]
    ...
    def _calculate_comprehensive_score(
        ...
        weights = {"indicator": 0.3, "complexity": 0.2, "innovation": 0.3, "risk_adjusted": 0.2}
        ...
        if (
            ...
            and innovation_score > 20
            and risk_adjusted_score > 40
        ):
