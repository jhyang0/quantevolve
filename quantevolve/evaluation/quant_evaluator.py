...
def evaluate(program_path: str) -> dict:
    ...
    try:
        ...
        )  # examples/quant_evolve -> examples -> project_root
        ...
        market_data_paths_to_try = [
            ...
            / "quant_evolve"
            / "data"
            / "btc_usdt_1h_2023.csv",  # If CWD is project root
            Path("data") / "btc_usdt_1h_2023.csv",  # If CWD is examples/quant_evolve
            Path(
                "../data/btc_usdt_1h_2023.csv"
            ),  # If CWD is inside a subfolder of quant_evolve (less likely)
        ]
        ...
        if not market_data_path:
            print(
                f"Error: Market data file not found. Tried: {[str(p) for p in market_data_paths_to_try]}"
            )
        ...
        market_data_df.dropna(
            subset=numeric_cols, inplace=True
        )  # Drop rows where essential data is NaN
        ...
        default_error_metrics["error_message"] = (
            f"Market data file not found at {str(market_data_path)}"
        )
        return default_error_metrics
    ...
    try:
        ...
        if not strategy_path.exists():
            ...
            default_error_metrics["error_message"] = (
                f"Strategy program file not found: {str(program_path)}"
            )
            return default_error_metrics
        ...
        if spec is None or spec.loader is None:
            ...
            default_error_metrics["error_message"] = (
                f"Could not create module spec for {str(program_path)}"
            )
            return default_error_metrics
        ...
        if not hasattr(strategy_module, "get_parameters"):
            ...
            default_error_metrics["error_message"] = (
                f"'get_parameters' missing in {str(program_path)}"
            )
            return default_error_metrics
        ...
        if not hasattr(strategy_module, "run_strategy"):
            ...
            default_error_metrics["error_message"] = (
                f"'run_strategy' missing in {str(program_path)}"
            )
            return default_error_metrics
        ...
        default_error_metrics["error_message"] = (
            f"Error loading strategy {str(program_path)}: {str(e)}"
        )
        return default_error_metrics
    ...
    try:
        ...
        print(tb_str)
        default_error_metrics["error_message"] = (
            f"Error running strategy {str(program_path)}: {str(e)}"
        )
        return default_error_metrics
    ...
    try:
        ...
        if (
            std_dev_strategy_returns == 0
            or np.isnan(std_dev_strategy_returns)
            or np.isinf(std_dev_strategy_returns)
        ):
            sharpe_ratio = (
                -100.0 if mean_strategy_return <= 0 else 0
            )  # Penalize if no variance but negative/zero mean
        else:
            sharpe_ratio = (
                mean_strategy_return * np.sqrt(annualization_factor)
            ) / std_dev_strategy_returns
        ...
        tb_str = traceback.format_exc()
        print(
            f"Error during backtesting/metrics calculation for {str(program_path)}: {str(e)}\n{tb_str}"
        )
        default_error_metrics["error_message"] = (
            f"Backtesting error for {str(program_path)}: {str(e)}"
        )
        return default_error_metrics
...
if __name__ == "__main__":
    ...
    if not expected_data_path.exists():
        alt_data_path = (
            Path(__file__).parent.parent.parent / "examples/quant_evolve/data/btc_usdt_1h_2023.csv"
        )
        if not alt_data_path.exists():
            print(
                f"Market data not found at expected locations like: {str(expected_data_path)} or {str(alt_data_path)}"
            )
            print(
                "Please ensure 'btc_usdt_1h_2023.csv' is in the 'examples/quant_evolve/data/' directory."
            )
            ...
            for i in range(100):  # ~4 days of hourly data
                ...
                dummy_df_content += (
                    f"{dt.isoformat()},{100+i*0.1},{101+i*0.1},{99+i*0.1},{100.5+i*0.1},{1000}\n"
                )
    ...
    if results_non_existent.get("error", 0.0) > 0.0:
        print(
            f"Error correctly handled for non-existent strategy: {str(results_non_existent.get('error_message'))}"
        )
    ...
    if results_bad_strategy.get("error", 0.0) > 0.0:
        print(
            f"Error correctly handled for bad strategy: {results_bad_strategy.get('error_message')}"
        )
