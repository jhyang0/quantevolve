...
def fetch_and_save_market_data(symbol: str, interval: str, start_str: str, end_str: str, filepath: str, config_path: str = None):
    """
    ...
    try:
        ...
        if not api_key or not api_secret:
            print("Error: Binance API key and/or secret not found in config or environment variables.")
            print("Please add them to configs/quantevolve_config.yaml under data_collection section")
        ...
        if "No module named 'binance'" in str(e):
            ...
            print("Please ensure your API key is correctly set and has trading/market data permissions.")
...
if __name__ == "__main__":
    ...
    if hasattr(config, 'data_collection') and config.data_collection:
        if hasattr(config.data_collection, 'default_symbol') and config.data_collection.default_symbol:
            default_symbol = config.data_collection.default_symbol
            
        if hasattr(config.data_collection, 'default_interval') and config.data_collection.default_interval:
        ...
        if hasattr(config.data_collection, 'default_lookback_days') and config.data_collection.default_lookback_days:
    ...
    interval_str = config.data_collection.default_interval if hasattr(config, 'data_collection') and hasattr(config.data_collection, 'default_interval') else '1h'
    year_str = str(datetime.now().year)
    default_filepath = f"data/{default_symbol.lower()}_{interval_str}_{year_str}.csv"
    
    print(f"Fetching data for {default_symbol} from {default_start_str} to {default_end_str} ({interval_str}).")
