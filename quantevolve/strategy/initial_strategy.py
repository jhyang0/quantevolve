...
def run_strategy(market_data_df: pd.DataFrame, params: list) -> pd.Series:
    ...
    if len(params) < 2:
        raise ValueError("SMA Crossover strategy requires at least two parameters: short_window and long_window.")
    ...
    market_data_df['short_sma'] = market_data_df['close'].rolling(window=short_window, min_periods=1).mean()

    # Calculate long-term SMA
    market_data_df['long_sma'] = market_data_df['close'].rolling(window=long_window, min_periods=1).mean()
    ...
    buy_signals = (prev_position < 1) & (position == 1) # short SMA crossed above long SMA (was <=, now >)
    
    # Generate sell signals
    sell_signals = (prev_position > -1) & (position == -1) # short SMA crossed below long SMA (was >=, now <)
...
if __name__ == "__main__":
    ...
    for i in range(1, data_size):
        ...
        close_prices[i] = close_prices[i-1] + np.random.randn() * 2 + (np.sin(i/5) * 2) # Add some trend and noise
